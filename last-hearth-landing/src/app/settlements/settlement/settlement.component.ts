...
export class SettlementComponent {
    ...
    protected readonly settlementInfo = this.settlementService.getSettlementInfo(this.userService.userId).pipe(tap((i) => {
        this.userService.getPlayer$(i.leader.user_id).pipe(
            tap((u) => {
    ...
    protected invitePlayer(info: ISettlement): void {
        this.dialogs.open(new PolymorpheusComponent(PlayerInviteComponent), { data: { settlementId: info.id } }).subscribe();
    }
