...
> **Базовый URL:** `https://api.lasthearth.ru` (прод) / из `environment.ts` > **Версия:** v1
...
interface ShopItem {
    ...
    privileges?: AbilityItem[]; // возможности / преимущества (в основном для привилегий)
...
interface AbilityItem {
    icon: string; // имя иконки, например `@tui.heart`
    text: string; // текст возможности / преимущества
}
```
