...
export class SettlementComponent {
    ...
    /**
     * Проверяет, может ли текущий пользователь исключать участников.
     *
     * Право исключения не выражается через `Permission`: по спеке
     * `DELETE /settlements/{id}/members/{user_id}` требует владельца
     * либо scope `settlements:manage` (администратор).
     *
     * @param settlement Поселение.
     * @returns true, если пользователь — владелец или администратор.
     */
    protected canRemoveMember(settlement: ISettlement): boolean {
        return isOwner(settlement, this.userId) || this.userService.roles.includes('admin');
    }
